export interface Review {
  slug: string;
  name: string;
  category: string;
  mealType: string[];
  price: string;
  rating: string;
  description: string;
  body?: string[];
  hours?: string;
  location?: string;
  websiteUrl?: string;
  image: string;
}

export const reviews: Review[] = [
  {
    slug: "frozen-rolls",
    name: "Frozen Rolls",
    category: "Dessert",
    mealType: ["Dessert"],
    price: "$",
    rating: "4.7/5 stars",
    description:
      "This Thai Ice Cream place is absolutely fantastic. Everything is made to order with three simple ingredients: Milk. Sugar. Cream.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "bird-bird-biscuit",
    name: "Bird Bird Biscuit",
    category: "American",
    mealType: ["Breakfast", "Brunch"],
    price: "$",
    rating: "4.8/5 stars",
    description: "Bird Bird is a great spot for a breakfast sandwich.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "franklins-bbq",
    name: "Franklin's BBQ",
    category: "BBQ",
    mealType: ["Lunch"],
    price: "$$",
    rating: "4.8/5 stars",
    description:
      "Franklin's BBQ is one of the most iconic spots in all of Austin. If you are not in line by 7:00am, you will not be served. The brisket is an absolute must but you can't go wrong with any of their meats.",
    body: [
      "Franklin's opens at 11am but the line starts forming around 7 to 8am, and it moves slowly until the meat runs out for the day. Get there by 10:30am if you want a real shot at brisket. Bring a chair, bring cash for the guy selling breakfast tacos and mimosas to the line, and treat the wait as part of the experience rather than an inconvenience.",
      "The brisket is sliced to order, wrapped in butcher paper, and weighed before it hits the famous blue tray. It's tender with a deep smoke ring and a dark, peppery bark, and it's the reason people fly into Austin specifically to eat here. Beyond brisket, the pork ribs, sausage, turkey, and pulled pork are all worth ordering, and the sides (cole slaw, pinto beans) are simple and done right.",
      "Parking near the East 11th St location is limited. If you're ordering 3-4+ pounds of meat, Franklin's offers online ordering with dedicated to-go parking spots, which skips the walk-up line entirely.",
    ],
    hours: "Tuesday-Sunday: 11am until sold out (closed Mondays)",
    location: "900 E 11th St, Austin, TX 78702",
    websiteUrl: "https://franklinbbq.com/",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "via-313",
    name: "Via 313",
    category: "Pizza",
    mealType: ["Lunch", "Dinner"],
    price: "$$",
    rating: "4.7/5 stars",
    description:
      "Via 313 brings Detroit-style pizza to Austin and does it incredibly well. The thick, crispy-edged, cheesy squares are a must-try.",
    body: [
      "Via 313 makes genuine Detroit-style pizza: a thick, airy square crust baked in steel pans, with cheese pushed all the way to the edges so it caramelizes into a crispy, almost fried border. Sauce goes on top in racing stripes after the bake, which is the classic Detroit move and part of what makes the crust stay crisp instead of soggy.",
      "The Detroiter (pepperoni, banana peppers, and their signature red sauce) is the one to start with if it's your first visit, but the menu also covers gluten-free and dairy-free crusts and a handful of solid salads if you're not going full pizza. It's been recognized by Food & Wine and Food Network as one of the best pizzas in the country, and named Independent Pizzeria of the Year by Pizza Today.",
      "Via 313 has grown to multiple locations across Austin, including East 6th Street, South Lamar, and 290/Oak Hill, so there's almost always one on your way somewhere. It's a solid pick for a casual group hang, since the pies are meant to be cut into squares and shared.",
    ],
    websiteUrl: "https://via313.com/",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "red-ash",
    name: "Red Ash",
    category: "Italian",
    mealType: ["Dinner"],
    price: "$$$",
    rating: "4.8/5 stars",
    description:
      "Red Ash is an upscale Italian restaurant in downtown Austin serving wood-fired dishes and handmade pasta.",
    body: [
      "Red Ash sits right on Colorado St in the heart of downtown, built around a wood-burning grill that does double duty for both the meats and a lot of the vegetables on the menu. The handmade pasta program is the real draw: the cappelletti, little \"bishop's hat\" shapes stuffed with spinach and sauced in brown butter, is the dish regulars keep coming back for.",
      "If you're going big, the 50-day dry-aged porterhouse (\"la Fiorentina\") is built for the table to share and shows off what the wood fire is capable of. It's a special-occasion price point, but the happy hour makes it easy to try Red Ash without committing to the full dinner: $2 off cocktails, beer, and wine by the glass, plus $7 appetizers, salads, and small pastas.",
      "Reservations move fast on weekends. If you want a walk-in shot at a table, arriving right around opening (5 to 5:15pm) gives you the best odds before the wait builds.",
    ],
    hours: "Mon-Sat: 5:00pm-10:00pm (hours vary Sunday, call ahead)",
    location: "303 Colorado St, Austin, TX 78701",
    websiteUrl: "https://www.redashgrill.com/",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "comedor",
    name: "Comedor",
    category: "Mexican",
    mealType: ["Dinner", "Brunch"],
    price: "$$$",
    rating: "4.7/5 stars",
    description:
      "Comedor offers elevated Mexican cuisine in a stunning modern setting in downtown Austin.",
    body: [
      "Comedor is led by chef Philip Speer, a James Beard semifinalist, and it's housed in a striking Tom Kundig-designed building of glass and steel right in downtown Austin. The kitchen builds its menu around heirloom masa and woodfire grilling, which shows up everywhere from the tortillas to the mains.",
      "The tacos are the move here: bone marrow with pecan gremolata, and fried fish with blistered cabbage, are both standouts and a good way to sample the kitchen's range in one sitting. Pair either with one of the smoky mezcal cocktails on the drink list.",
      "It's a dressier spot suited to a date night or a client dinner rather than a casual drop-in, and reservations are worth booking ahead given the design-forward dining room draws a steady crowd. Seating includes an indoor dining room, an open courtyard patio with retractable elements, and bar seating if you'd rather walk in and grab a mezcal.",
      "Comedor picked up Best New Restaurant in Texas from Texas Monthly and has been covered by the New York Times, Wall Street Journal, and Michelin. Sunday brunch (10am-3pm) is a quieter, more casual way to try the kitchen if the full dinner service feels like too much of a commitment.",
    ],
    hours: "Mon-Thu: 5:00pm-10:00pm, Fri-Sat: 5:00pm-11:00pm, Sun: 10:00am-3:00pm (brunch) & 5:00pm-9:00pm",
    location: "501 Colorado St, Austin, TX 78701",
    websiteUrl: "https://comedortx.com/",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "pueblo-viejo",
    name: "Pueblo Viejo",
    category: "Mexican",
    mealType: ["Breakfast", "Lunch"],
    price: "$",
    rating: "4.8/5 stars",
    description:
      "Pueblo Viejo is a beloved Austin food truck serving authentic Mexican breakfast tacos and more.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "ramen-tatsu-ya",
    name: "Ramen Tatsu-ya",
    category: "Asian",
    mealType: ["Lunch", "Dinner"],
    price: "$$",
    rating: "4.9/5 stars",
    description:
      "Ramen Tatsu-ya is one of the best ramen spots in all of Texas with rich, flavorful broths and perfect noodles.",
    body: [
      "Ramen Tatsu-ya started in 2012 as a single spot on East 6th Street from chef and DJ Tatsu Aikawa, and it's grown into what a lot of Austinites just call their Japanese soul food. The broth is the whole point: a 60-hour tonkotsu that's rich without being heavy, and it's customizable with \"bombs,\" their term for concentrated flavor add-ins that let you dial in exactly how funky or spicy you want the bowl.",
      "Beyond ramen, the katsu sliders, karaage (Japanese fried chicken), and gyoza are all worth ordering as a table to share while you wait on bowls. Happy hour runs Monday through Friday from 3 to 5pm if you're going for drinks and small plates rather than a full meal.",
      "The original East 6th St location still has the neighborhood-favorite feel that built the brand, but Ramen Tatsu-ya has since expanded to multiple spots around Austin (and into Houston), so there's usually one within reach no matter where you are in the city.",
    ],
    hours: "Original location, Mon-Thu: 11am-10pm, Fri: 11am-11pm, Sat: 11am-10pm, Sun: 12pm-10pm",
    location: "1600 E 6th St, Austin, TX 78702 (original location; multiple Austin locations)",
    websiteUrl: "https://www.ramen-tatsuya.com/",
    image: "/images/food-photo-1.jpg",
  },

  // --- Seed batch pulled from Austin Food Heads' own social content history (ClickUp) ---
  // Lighter directory-format entries: no full write-up yet, so no individual detail
  // page is generated for these (see generateStaticParams in [slug]/page.tsx).
  {
    slug: "la-barbecue",
    name: "La Barbecue",
    category: "BBQ",
    mealType: ["Lunch"],
    price: "$$",
    rating: "4.7/5 stars",
    description:
      "Austin's other essential BBQ trailer, known for brisket and peppery, moist sausage links that give Franklin's a real run for its money.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "camino-alamo-bbq",
    name: "Camino Alamo BBQ",
    category: "BBQ",
    mealType: ["Lunch", "Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description:
      "Central Texas smoked meats with a laid-back neighborhood feel, a solid pick when the Franklin's line isn't in the cards.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "texazuela-bbq",
    name: "TexaZuela BBQ",
    category: "BBQ",
    mealType: ["Lunch", "Dinner"],
    price: "$$",
    rating: "4.7/5 stars",
    description:
      "Texas smoked meat meets Venezuelan flavors, brisket next to arepas and tequenos on the same tray.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "juan-in-a-million",
    name: "Juan in a Million",
    category: "Mexican",
    mealType: ["Breakfast"],
    price: "$",
    rating: "4.7/5 stars",
    description:
      "An East Austin breakfast taco institution, home of the massive \"Don Juan\" taco that regulars have been ordering for decades.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "vaqueros",
    name: "Vaqueros",
    category: "Mexican",
    mealType: ["Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description:
      "A modern Mexican kitchen with a solid margarita list, good for a casual group dinner that's still a step up from tacos on the go.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "los-chilakillers",
    name: "Los Chilakillers",
    category: "Mexican",
    mealType: ["Breakfast", "Lunch"],
    price: "$",
    rating: "4.6/5 stars",
    description:
      "A chilaquiles specialist that turns a usually-simple dish into the whole reason to show up.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "cabo-bobs",
    name: "Cabo Bob's",
    category: "Mexican",
    mealType: ["Lunch", "Dinner"],
    price: "$",
    rating: "4.5/5 stars",
    description:
      "Baja-style fish tacos and burritos, quick, casual, and reliable across its Austin locations.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "micas-burritos",
    name: "Mica's Burritos",
    category: "Mexican",
    mealType: ["Breakfast", "Lunch"],
    price: "$",
    rating: "4.7/5 stars",
    description:
      "A breakfast burrito food truck with a cult following, order ahead if you can, mornings get busy.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "slice-street-pizza",
    name: "Slice Street Pizza",
    category: "Pizza",
    mealType: ["Lunch", "Dinner"],
    price: "$$",
    rating: "4.7/5 stars",
    description:
      "New York-style slices done right, a solid grab-and-go option when you don't want the full Detroit-style production.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "garage-pizza",
    name: "Garage Pizza",
    category: "Pizza",
    mealType: ["Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "A neighborhood pizza spot with a laid-back garage-bar feel to match the name.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "moderna-pizzeria",
    name: "Moderna Pizzeria",
    category: "Pizza",
    mealType: ["Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "Wood-fired, Neapolitan-leaning pies in a casual setting.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "outlaw-pizza",
    name: "Outlaw Pizza",
    category: "Pizza",
    mealType: ["Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "A solid local pizza option worth keeping in the weeknight rotation.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "ciccio-bomba",
    name: "Ciccio Bomba",
    category: "Italian",
    mealType: ["Dinner"],
    price: "$$$",
    rating: "4.7/5 stars",
    description: "A dressed-up Italian spot for pasta and a proper date-night dinner.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "its-italian-cucina",
    name: "It's Italian Cucina",
    category: "Italian",
    mealType: ["Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "Comfortable, no-frills Italian classics done well.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "sushi-by-scratch",
    name: "Sushi by Scratch",
    category: "Japanese",
    mealType: ["Dinner"],
    price: "$$$$",
    rating: "4.9/5 stars",
    description:
      "An intimate, counter-only omakase concept, book ahead, it's one of the harder-to-get reservations in town.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "shokunin",
    name: "Shokunin",
    category: "Japanese",
    mealType: ["Dinner"],
    price: "$$$",
    rating: "4.7/5 stars",
    description: "Japanese small plates and a serious sake and whiskey list for a slower dinner.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "kitsu-nori",
    name: "Kitsu Nori",
    category: "Japanese",
    mealType: ["Lunch", "Dinner"],
    price: "$$",
    rating: "4.7/5 stars",
    description: "A reliable sushi spot for rolls and bowls without the special-occasion price tag.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "aburi-tora",
    name: "Aburi Tora",
    category: "Japanese",
    mealType: ["Dinner"],
    price: "$$",
    rating: "4.7/5 stars",
    description: "Torched, seared-style sushi that's a fun twist if you're bored of the usual roll menu.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "boa-steakhouse",
    name: "BOA Steakhouse",
    category: "Steakhouse",
    mealType: ["Dinner"],
    price: "$$$$",
    rating: "4.7/5 stars",
    description: "A big-night-out steakhouse with the polish you'd expect from a special-occasion splurge.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "vince-young-steakhouse",
    name: "Vince Young Steakhouse",
    category: "Steakhouse",
    mealType: ["Dinner"],
    price: "$$$",
    rating: "4.6/5 stars",
    description:
      "A Texas-sized steakhouse from the former Longhorns quarterback, solid for a celebratory dinner.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "oseyo",
    name: "Oseyo",
    category: "Korean",
    mealType: ["Dinner"],
    price: "$$",
    rating: "4.7/5 stars",
    description: "Korean fried chicken and comfort food with a solid drink menu to match.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "banjoo",
    name: "Banjoo",
    category: "Korean",
    mealType: ["Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "Korean classics in a casual, no-fuss setting.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "puradak",
    name: "Puradak Fried Chicken",
    category: "Korean",
    mealType: ["Lunch", "Dinner"],
    price: "$",
    rating: "4.6/5 stars",
    description: "Crispy, double-fried Korean fried chicken, quick service and dangerously easy to over-order.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "another-broken-egg-cafe",
    name: "Another Broken Egg Cafe",
    category: "American",
    mealType: ["Breakfast", "Brunch"],
    price: "$$",
    rating: "4.5/5 stars",
    description: "A reliable weekend brunch chain option when the wait at the local spots is too long.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "jps-pancake-company",
    name: "JP's Pancake Company",
    category: "American",
    mealType: ["Breakfast"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "Pancakes are the whole point here, stacked, stuffed, and worth the sugar crash.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "maple-street-biscuit-company",
    name: "Maple Street Biscuit Company",
    category: "American",
    mealType: ["Breakfast"],
    price: "$",
    rating: "4.6/5 stars",
    description: "Southern biscuit sandwiches, fast and satisfying before a full day out.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "paperboy",
    name: "Paperboy",
    category: "American",
    mealType: ["Breakfast", "Brunch"],
    price: "$$",
    rating: "4.8/5 stars",
    description:
      "A tiny East Austin breakfast spot with an outsized reputation, expect a wait and a menu worth it.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "penelopes-cafe",
    name: "Penelope's Cafe",
    category: "American",
    mealType: ["Breakfast", "Brunch"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "A cozy neighborhood brunch spot, good coffee and an easy weekend pace.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "hopdoddy",
    name: "Hopdoddy Burger Bar",
    category: "American",
    mealType: ["Lunch", "Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "The gourmet burger chain that started in Austin, still a solid, reliable pick.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "smashed-n-sauced",
    name: "Smashed N Sauced",
    category: "American",
    mealType: ["Lunch", "Dinner"],
    price: "$",
    rating: "4.6/5 stars",
    description: "Smash burgers done right, thin, crispy-edged patties and a proper sauce game.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "tumble22",
    name: "Tumble 22",
    category: "American",
    mealType: ["Lunch", "Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "Nashville hot chicken and wings for when you want spice with your comfort food.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "black-sheep-coffee",
    name: "Black Sheep Coffee",
    category: "Coffee",
    mealType: ["Breakfast", "Drinks"],
    price: "$",
    rating: "4.5/5 stars",
    description: "A dependable coffee stop for a morning meeting or a work-from-cafe afternoon.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "rocheli-patisserie",
    name: "Rocheli Patisserie",
    category: "Dessert",
    mealType: ["Dessert"],
    price: "$$",
    rating: "4.8/5 stars",
    description: "French pastries and cakes worth the drive, a proper treat-yourself stop.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "the-crepe-lab",
    name: "The Crepe Lab",
    category: "Dessert",
    mealType: ["Dessert", "Brunch"],
    price: "$",
    rating: "4.6/5 stars",
    description: "Sweet and savory crepes made fresh to order, a fun, quick dessert stop.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "easy-tiger",
    name: "Easy Tiger",
    category: "American",
    mealType: ["Breakfast", "Drinks"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "German-style bakery by day, beer garden by night, right along the hike-and-bike trail.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "cenote",
    name: "Cenote",
    category: "American",
    mealType: ["Breakfast", "Brunch"],
    price: "$$",
    rating: "4.6/5 stars",
    description:
      "An East Austin cafe favorite that just reopened at a new East 7th St location after its old spot closed.",
    location: "1405 E 7th St, Austin, TX 78702",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "skipjack-oyster-bar",
    name: "SkipJack Oyster Bar",
    category: "Seafood",
    mealType: ["Dinner"],
    price: "$$$",
    rating: "4.6/5 stars",
    description: "Fresh oysters and seafood in a lively bar setting.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "central-machine-works",
    name: "Central Machine Works",
    category: "Brewery",
    mealType: ["Drinks"],
    price: "$$",
    rating: "4.7/5 stars",
    description: "A brewery and beer garden in a converted machine shop, good for a low-key hang with friends.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "lazarus-brewing",
    name: "Lazarus Brewing",
    category: "Brewery",
    mealType: ["Drinks"],
    price: "$$",
    rating: "4.7/5 stars",
    description: "A local brewery with a solid rotating tap list and an easygoing taproom.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "rosewood-social-club",
    name: "Rosewood Social Club",
    category: "American",
    mealType: ["Dinner", "Drinks"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "A neighborhood bar and kitchen combo, good for drinks that turn into dinner.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "sangam-chettinad",
    name: "Sangam Chettinad",
    category: "Indian",
    mealType: ["Lunch", "Dinner"],
    price: "$$",
    rating: "4.7/5 stars",
    description: "South Indian Chettinad cooking, spicier and less commonly found than typical Austin Indian spots.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "fig-and-brie",
    name: "Fig and Brie",
    category: "American",
    mealType: ["Dinner"],
    price: "$$$",
    rating: "4.7/5 stars",
    description: "An upscale bistro with a menu built around seasonal, shareable plates.",
    image: "/images/food-photo-3.jpg",
  },
  {
    slug: "geraldines",
    name: "Geraldine's",
    category: "American",
    mealType: ["Dinner", "Brunch"],
    price: "$$$",
    rating: "4.7/5 stars",
    description: "Southern-leaning small plates and live music inside Hotel Van Zandt, dressy but not stuffy.",
    image: "/images/hero-food.jpg",
  },
  {
    slug: "dish-society",
    name: "Dish Society",
    category: "American",
    mealType: ["Breakfast", "Lunch", "Dinner"],
    price: "$$",
    rating: "4.5/5 stars",
    description: "A farm-to-table all-day menu that covers breakfast through dinner in one stop.",
    image: "/images/food-photo-1.jpg",
  },
  {
    slug: "woodneck-kitchen",
    name: "Woodneck Kitchen",
    category: "American",
    mealType: ["Dinner"],
    price: "$$",
    rating: "4.6/5 stars",
    description: "A neighborhood kitchen with a rotating seasonal menu worth checking back on.",
    image: "/images/food-photo-2.jpg",
  },
  {
    slug: "jewboy-sliders",
    name: "Jewboy Sliders",
    category: "American",
    mealType: ["Lunch"],
    price: "$",
    rating: "4.7/5 stars",
    description: "A food truck turned local favorite for Jewish deli-inspired sliders and smoked meats.",
    image: "/images/food-photo-3.jpg",
  },
];
